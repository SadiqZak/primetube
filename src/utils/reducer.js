const reducerFunc = (state, action) => {
  switch (action.type) {
    case "initialState":
      // const handleLiked =()=>{
      //     return state.videoLib.map(({id})=>{
      //       return {id:id, bool:false}
      //     })
      // }
      return {
        ...state,
        ...
        // videodetailState:{...state.videodetailState, LikedVideos:handleLiked()}
      };
    case "All":
      return {
        ...state,
    ...
    case "WatchHistory":
      const historyUpdate=()=>{
        ...
        const updatedHistoryList = state.history.find((item)=>item.id===action.payload.id)
